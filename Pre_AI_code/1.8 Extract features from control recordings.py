    ...
    while True:
        ...
        if len(peaks_found) != 0:
            ...
            if n_eods != 0:
                ...
                plt.savefig('%s\\%s_%s_%s.png'%(output_path, species, fish_id, fname[:-4]))
                aio.write_audio('%s\\%s_%s_%s.wav'%(output_path, species, fish_id, fname[:-4]), data, rate)
                
                eod_table_complete.to_csv('%s\\%s_%s_%s_eod_table.csv'%(output_path, species, fish_id, fname[:-4]), index=False )
                waveform_table = pd.DataFrame(filtered_h2t_waveforms)
                waveform_table.to_csv('%s\\%s_%s_%s_h2t_waveforms.csv'%(output_path, species, fish_id, fname[:-4]), index=False )
        ...
        if repeat == 0:
            ...
            while change_params:
                ...
                if done:
                    ...
                    parameters = {k:[v] for k,v in parameters.items()}  # WORKAROUND
                    parameters = pd.DataFrame(parameters)
                    parameters.to_csv('%s\\%s_analysis_parameters.csv'%(n, output_path), index=False )
                else:
                    change_params = 1
                    plt.close()
