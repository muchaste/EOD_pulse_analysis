...
def remove_proximity_duplicates(arrays_dict, proximity_threshold=3):
    ...
    # the threshold — same-channel neighbours in that window are duplicates.
    ...
    unique_mask[sort_order] = unique_mask_sorted
...
def reclassify_session(input_path, output_path, species_classifier, lda_min_probability=0.0):
    """
    Reclassify all fish already tracked in `input_path` (a 04_1_Track_Fish.py session
    output folder) against a fitted `species_classifier` (see fit_species_classifier),
    and write a new session at `output_path` - without re-running tracking. Reuses each
    fish's stored mean waveform from that session's session_*_mean_waveforms.npz files.
    Shared by 04_1b_Reclassify_Species.py (single session, GUI-driven) and
    04_1c_Batch_Reclassify_Species.py (loops this over many sessions).

    Parameters
    ----------
    input_path : str
        Session folder containing tracked_fish_summary.csv, tracked_event_summary.csv
        (optional), and session_*_mean_waveforms.npz/_keys.csv files.
    output_path : str
        Folder to write the new session outputs to (created if missing).
    species_classifier : dict
        Output of fit_species_classifier().
    lda_min_probability : float, optional
        Passed through to classify_fish_waveform().

    Returns
    -------
    fish_summary_df : DataFrame or None
        The new per-fish summary (also written to output_path), or None if the session had
        no usable tracked fish / stored mean waveforms.
    """
    fish_summary_path = os.path.join(input_path, 'tracked_fish_summary.csv')
    if not os.path.exists(fish_summary_path):
        print(f"⚠ No tracked_fish_summary.csv in {input_path}")
        return None
    ...
    if fish_summary_df.empty:
        print(f"⚠ No usable tracked fish in {input_path}")
        return None
    ...
    if os.path.exists(orig_event_summary_path):
