...
ELECTRODE_METADATA_FILE <- "eod_logger_4/site_electrode_metadata.csv"  # electrode index -> physical position lookup
...
# ---- Electrode/site physical position metadata --------------------------------
# Maps electrode index (0-7) to physical meaning (shore distance, depth, flow position).
# LN loggers are reused across sessions and matched by Placement only, not Logger_ID.
electrode_metadata <- read.csv(ELECTRODE_METADATA_FILE, stringsAsFactors = FALSE)

electrode_metadata_specific <- electrode_metadata |> filter(System != "LN")
electrode_metadata_LN       <- electrode_metadata |> filter(System == "LN") |> select(-Site, -Logger_ID)

register_specific <- register_raw |>
  filter(System != "LN") |>
  left_join(electrode_metadata_specific, by = c("System", "Site", "Logger_ID", "Placement"))

register_LN <- register_raw |>
  filter(System == "LN") |>
  left_join(electrode_metadata_LN, by = c("System", "Placement"))

register_raw <- bind_rows(register_specific, register_LN)
...
for(ft_file in fish_track_list){
  ...
  ft_dat$array_type <- register_raw$array_type[register_raw$Session_date == ft_dat$Session_date[1] & register_raw$Logger_ID == ft_dat$Logger_id[1]]
  ft_dat$gradient_variable <- register_raw$gradient_variable[register_raw$Session_date == ft_dat$Session_date[1] & register_raw$Logger_ID == ft_dat$Logger_id[1]]
  ft_dat$ch0_meaning <- register_raw$ch0_meaning[register_raw$Session_date == ft_dat$Session_date[1] & register_raw$Logger_ID == ft_dat$Logger_id[1]]
  ft_dat$gradient_endpoint_m <- register_raw$gradient_endpoint_m[register_raw$Session_date == ft_dat$Session_date[1] & register_raw$Logger_ID == ft_dat$Logger_id[1]]
...
# electrode index -> physical distance/depth, oriented so 0 m = ch0_meaning
fish_raw$location_m <- fish_raw$mean_location * (fish_raw$gradient_endpoint_m / 7)
...
for(ev_file in event_list){
  ...
  ev_dat$array_type <- register_raw$array_type[register_raw$Session_date == ev_dat$Session_date[1] & register_raw$Logger_ID == ev_dat$Logger_id[1]]
  ev_dat$gradient_variable <- register_raw$gradient_variable[register_raw$Session_date == ev_dat$Session_date[1] & register_raw$Logger_ID == ev_dat$Logger_id[1]]
...
ts_expanded <- fish_filt |>
  ...
  ungroup() |>
  select(fish_id, species_assigned, Site, System, Placement, array_type, gradient_variable,
         Session_date, Logger_id, Sunrise, Sunset, Sunrise_next, Sunset_next, datetime) |>
  unnest(datetime)

ts_new <- ts_expanded |>
  count(Site, System, Placement, array_type, gradient_variable, Session_date, Logger_id,
